...
class Agent(nn.Module):
    def __init__(self, in_features: int, out_features: int):
        ...
        # Critic
        self.critic_oa_encoder = nn.Linear(in_features + out_features, 128)
        self._critic_prev_a_encoer = nn.Linear(out_features, 32)
        self._critic_r_encoder = nn.Linear(1, 32)
        self._critic_o_encoder = nn.Linear(in_features, 64)
        self.critic_aro_mixer = nn.Linear(128, 128)
        ...
        # Actor
        self.actor_o_encoder = nn.Linear(in_features, 128)
        self._actor_prev_a_encoer = nn.Linear(out_features, 32)
        self._actor_r_encoder = nn.Linear(1, 32)
        self._actor_o_encoder = nn.Linear(in_features, 64)
        self.actor_aro_mixer = nn.Linear(128, 128)
    ...
    def get_value(
        ...
        x = th.tanh(self.critic_oa_encoder(x))

        _a = th.tanh(self._critic_prev_a_encoer(prev_action))
        _r = th.tanh(self._critic_r_encoder(reward))
        _o = th.tanh(self._critic_o_encoder(obs))
        aro = th.tanh(self.critic_aro_mixer(th.cat([_a, _r, _o], dim=-1)))

        value = self.critic(th.cat([x, aro], dim=-1))
    ...
    def _get_actor_mean(
        ...
        x = th.tanh(self.actor_o_encoder(obs))

        _a = th.tanh(self._actor_prev_a_encoer(prev_action))
        _r = th.tanh(self._actor_r_encoder(reward))
        _o = th.tanh(self._actor_o_encoder(obs))
        aro = th.tanh(self.actor_aro_mixer(th.cat([_a, _r, _o], dim=-1)))

        actor_mean = self.actor_mean(th.cat([x, aro], dim=-1))
        return actor_mean
...
def train_ppo(args: Args, Agent: type[Agent]) -> None:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # Get action
            with th.no_grad():
                action, logprob, _ = agent.get_action(obs, prev_action, reward)
                value = agent.get_value(obs, prev_action, reward, action)  # (num_envs, 1)
            ...
            for idx, truncation in enumerate(truncations):
                if truncation:
                    ...
                    final_reward = reward[idx].unsqueeze(0)  # (1, 1)
                    with th.no_grad():
    ...
    if args.save_model:
        ...
        from rlink.evaluators.mujoco.eval_ppo_mujoco_mlp_aro import ppo_evaluate
