...
def HalfCheetah_P_factory() -> gym.Env:
    env_id = "HalfCheetah-v4"
    visible_obs_dim = [0, 1, 2, 3, 4, 5, 6, 7]
    env = POMDPWrapper(env_id, visible_obs_dim)
    return env


def HalfCheetah_V_factory() -> gym.Env:
    env_id = "HalfCheetah-v4"
...
def Walker2D_P_factory() -> gym.Env:
    env_id = "Walker2D-v4"
    visible_obs_dim = [0, 1, 2, 3, 4, 5, 6, 7]
    env = POMDPWrapper(env_id, visible_obs_dim)
    return env
...
def Walker2D_V_factory() -> gym.Env:
    env_id = "Walker2D-v4"
    visible_obs_dim = [8, 9, 10, 11, 12, 13, 14, 15, 16]
    env = POMDPWrapper(env_id, visible_obs_dim)
    return env
